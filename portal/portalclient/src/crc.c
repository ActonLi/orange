...
void crc_calcvalue(U8 val, U8* crc)
{
	U8 uIndex;
	uIndex = crc[0] ^ val;
	crc[0] = crc[1] ^ crchigh[uIndex];
	crc[1] = crclow[uIndex];
}
void crc_calc(U8* msg, U16 length, U8* crc)
{
	U8  uchCRCHi = 0xFF;
	U8  uchCRCLo = 0xFF;
	U16 DataLen;
	...
	while (DataLen--) {
		uIndex   = (uchCRCHi ^ *msg);
		uchCRCHi = uchCRCLo ^ crchigh[uIndex];
		uchCRCLo = crclow[uIndex];
		msg++;
	}
	*crc = uchCRCHi;
	crc++;
