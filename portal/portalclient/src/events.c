...
void event_post(U32 semid, char* module, EnumEventType eventtype, void* param, U32 size, U32 timeout)
{
	...
	if (NULL != sem) {
		if (module != NULL) {
			printf("event post modulename=%s\n", module);
			last->semid = semid;
...
void event_clearflags(U32 semid, STREventElemType* event, U32 moduleid)
{
	...
	OSA_MutexLock(&g_event_lk);
	/////printf("event clearflags moduleid=%d, mask=%08x\n", moduleid, mask);
	if (event->this != NULL) {
		event->this->flags &= mask;
	}
	/////printf("event clearflags after clear moduleid=%d, flags=%08x\n", moduleid, event->this->flags);
	OSA_MutexUnlock(&g_event_lk);
}
...
BOOL event_get_jsonmsg(void* params, MSG_DATA_HEADER* header, cJSON** json, U32* msgid)
{
	BOOL  res = FALSE;
	char* payload;
	char* p_params_4 = ((char*) params) + 4;
	*msgid			 = (U32) * ((U32*) params);
	memcpy(header, p_params_4, sizeof(MSG_DATA_HEADER));
	payload = ((char*) params) + (4 + MSG_HEADER_LENTH);
