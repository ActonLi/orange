 ...
 */
NOPOLL_SOCKET nopoll_conn_sock_connect(noPollCtx* ctx, const char* host, const char* port)
{
...
char* __nopoll_conn_get_client_init(noPollConn* conn)
{
	...
	return nopoll_strdup_printf(
		...
		conn->origin,
		/* protocol part */
		conn->protocols ? "Sec-WebSocket-Protocol" : "", conn->protocols ? ": " : "", conn->protocols ? conn->protocols : "", conn->protocols ? "\r\n" : "",
...
int nopoll_conn_log_ssl(noPollConn* conn)
{
	...
	nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "    noPoll id=%d, socket: %d (after testing errno: %d)", conn->id, conn->session, errno);
...
int __nopoll_conn_tls_handle_error(noPollConn* conn, int res, const char* label, nopoll_bool* needs_retry)
{
	...
	switch (ssl_err) {
		...
		case SSL_ERROR_WANT_X509_LOOKUP:
			nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "%s, ssl_err=%d returned that isn't ready to read/write: you should retry", label, ssl_err);
		...
		case SSL_ERROR_SYSCALL:
			...
			nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "SSL socket closed on %s (res=%d, ssl_err=%d, errno=%d)", label, res, ssl_err, errno);
		...
		case SSL_ERROR_SSL:
			...
			nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "%s function error (received SSL_ERROR_SSL) (res=%d, ssl_err=%d, errno=%d)", label, res, ssl_err,
					   errno);
...
int nopoll_conn_tls_send(noPollConn* conn, char* buffer, int buffer_size)
{
	...
	nopoll_bool needs_retry;
	int			tries = 0;
	// printf("nopoll_conn_tls_send begin %d \n",time(NULL));
	/* call to read content */
	while (tries < 50) {
	...
	// printf("ssl write tries %d \n",tries);
	// printf("nopoll_conn_tls_send end %d \n",time(NULL));
	return res;
}
 ...
 */
noPollConn* __nopoll_conn_new_common(noPollCtx* ctx, nopoll_bool enable_tls, const char* host_ip, const char* host_port, const char* host_name,
									 const char* get_url, const char* protocols, const char* origin)
{
	noPollConn*   conn;
	NOPOLL_SOCKET session;
	char*		  content;
	...
	nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "Created noPoll conn-id=%d (ptr: %p, context: %p, socket: %d)", conn->id, conn, ctx, session);
	...
	if (enable_tls) {
		...
		the misconception that non-blocking SSL_write() behaves like non-blocking write().*/
		SSL_CTX_set_mode(conn->ssl_ctx, SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER);
		if (ctx->certificates != NULL) {
			nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "certificate path: %s", ctx->certificates->certificateFile);

			if (SSL_CTX_use_certificate_file(conn->ssl_ctx, ctx->certificates->certificateFile, SSL_FILETYPE_PEM) <= 0) {
				printf("Error setting the certificate file.\n");
				nopoll_conn_close(conn);
				return NULL;
			}

			nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "certificate path: %s", ctx->certificates->privateKey);
			/*Indicate the key file to be used*/
			if (SSL_CTX_use_PrivateKey_file(conn->ssl_ctx, ctx->certificates->privateKey, SSL_FILETYPE_PEM) <= 0) {
				printf("Error setting the key file.\n");
				nopoll_conn_close(conn);
				return NULL;
			}

			if (ctx->certificates->optionalChainFile != NULL) {
				SSL_CTX_set_verify(conn->ssl_ctx, SSL_VERIFY_PEER, NULL);
				if (!SSL_CTX_load_verify_locations(conn->ssl_ctx, ctx->certificates->optionalChainFile, NULL)) {
					printf("Error setting the CA certificate file.\n");
					nopoll_conn_close(conn);
					return NULL;
				}
			}
		...
		// int ret=setsockopt(conn->session,SOL_SOCKET,SO_SNDTIMEO,&timeout,sizeof(timeout));
		// printf("set send timeout  = %d\n",ret);
		// ret=setsockopt(conn->session,SOL_SOCKET,SO_RCVTIMEO,&timeout,sizeof(timeout));
		// printf("set recv timeout  = %d\n",ret);
		// perror("err\n");
		...
		while (SSL_connect(conn->ssl) <= 0) {
			...
			switch (ssl_error) {
				case SSL_ERROR_WANT_READ:
					...
					break;
				case SSL_ERROR_WANT_WRITE:
					nopoll_log(ctx, NOPOLL_LEVEL_WARNING, "still not prepared to continue because write wanted, conn-id=%d (%p)", conn->id, conn);
					break;
				case SSL_ERROR_SYSCALL:
					nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL,
							   "syscall error while doing TLS handshake, ssl error (code:%d), conn-id: %d (%p), errno: %d, session: %d", ssl_error, conn->id,
							   conn, errno, conn->session);
					nopoll_conn_log_ssl(conn);
					nopoll_conn_close(conn);
					return NULL;
				default:
					nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "there was an error with the TLS negotiation, ssl error (code:%d) : %s", ssl_error,
			...
			if (iterator > 80) {
				nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Max retry calls=%d to SSL_connect reached, shutting down connection id=%d, errno=%d", iterator,
						   conn->id, errno);
				nopoll_conn_close(conn);
				return NULL;
			} /* end if */
 ...
 * space).
 */
noPollConn* nopoll_conn_new(noPollCtx* ctx, const char* host_ip, const char* host_port, const char* host_name, const char* get_url, const char* protocols,
							const char* origin)
{
	/* call common implementation */
	return __nopoll_conn_new_common(ctx, nopoll_false, host_ip, host_port, host_name, get_url, protocols, origin);
}
 ...
 *
 */
noPollConn* nopoll_conn_tls_new(noPollCtx* ctx, noPollPtr tls_options, const char* host_ip, const char* host_port, const char* host_name, const char* get_url,
								const char* protocols, const char* origin)
{
	...
	/* call common implementation */
	return __nopoll_conn_new_common(ctx, nopoll_true, host_ip, host_port, host_name, get_url, protocols, origin);
}
...
/* report connection close */
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "shutting down connection id=%d (session: %d, role: %s)", conn->id, conn->session, role);
#endif
...
void nopoll_conn_close(noPollConn* conn)
{
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Calling to close close id=%d (session %d, refs: %d, role: %s)", conn->id, conn->session, conn->refs, role);
#endif
	if (conn->session) {
...
void nopoll_conn_unref(noPollConn* conn)
{
	...
	conn->refs--;
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Releasing connection id %d reference, current ref count status is: %d", conn->id, conn->refs);
	if (conn->refs != 0) {
		/* release here the mutex */
		nopoll_mutex_unlock(conn->ref_mutex);
		return;
...
int nopoll_conn_readline(noPollConn* conn, char* buffer, int maxlen)
{
	...
	if (conn->pending_line) {
		...
		if (desp >= maxlen) {
			nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "found fragmented frame line header but allowed size was exceeded (desp:%d >= maxlen:%d)", desp, maxlen);
			nopoll_conn_shutdown(conn);
			return -1;
		} /* end if */
	...
	nopoll_readline_again:
		if ((rc = conn->receive(conn, &c, 1)) == 1) {
			*ptr++ = c;
			...
			nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "unable to read line, error code errno: %d, rc: %d (%s)", errno, rc, strerror(errno));
			return (-1);
		}
	}
	*ptr = 0;
...
int __nopoll_conn_receive(noPollConn* conn, char* buffer, int maxlen)
{
	...
	if (conn->pending_buf_bytes > 0) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Calling with bytes we can reuse (%d), requested: %d", conn->pending_buf_bytes, maxlen);
...
/* memset (buffer, 0, maxlen * sizeof (char )); */
	...
	if (nread == 0) {
		...
		if (errno == NOPOLL_EAGAIN || errno == NOPOLL_EWOULDBLOCK) {
			nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "unable to read from conn-id=%d (%s:%s), connection is not ready (errno: %d : %s)", conn->id,
					   conn->host, conn->port, errno, strerror(errno));
			return 0;
		} /* end if */

		nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL,
				   "received connection close while reading from conn id %d (errno=%d : %s) (%d, %d, %d), shutting down connection..", conn->id, errno,
				   strerror(errno), NOPOLL_EAGAIN, NOPOLL_EWOULDBLOCK, NOPOLL_EINTR);
		nopoll_conn_shutdown(conn);
	} /* end if */
 ...
 */
nopoll_bool nopoll_conn_check_mime_header_repeated(noPollConn* conn, char* header, char* value, const char* ref_header, noPollPtr check)
{
...
nopoll_bool nopoll_conn_complete_handshake_check_listener(noPollCtx* ctx, noPollConn* conn)
{
	...
	if (!conn->handshake->upgrade_websocket || !conn->handshake->connection_upgrade || !conn->handshake->websocket_key || !conn->origin ||
		!conn->handshake->websocket_version) {
		nopoll_log(
			ctx, NOPOLL_LEVEL_CRITICAL,
			"Client from %s:%s didn't provide all websocket handshake values required, closing session (Upgraded: websocket %d, Connection: upgrade%d, Sec-WebSocket-Key: %p, Origin: %p, Sec-WebSocket-Version: %p)",
			conn->host, conn->port, conn->handshake->upgrade_websocket, conn->handshake->connection_upgrade, conn->handshake->websocket_key, conn->origin,
			conn->handshake->websocket_version);
		return nopoll_false;
	} /* end if */
	...
	if (strtod(conn->handshake->websocket_version, NULL) != ctx->protocol_version) {
		nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Received request for an unsupported protocol version: %s, expected: %d", conn->handshake->websocket_version,
	...
	if (ctx->on_open) {
		if (!ctx->on_open(ctx, conn, ctx->on_open_data)) {
			nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Client from %s:%s was denied by application level, clossing session", conn->host, conn->port);
			nopoll_conn_shutdown(conn);
			return nopoll_false;
		}
	} /* end if */

	nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "Client from %s:%s was accepted, replying accept", conn->host, conn->port);
...
nopoll_bool nopoll_conn_complete_handshake_check_client(noPollCtx* ctx, noPollConn* conn)
{
	...
	if (!conn->handshake->websocket_accept || !conn->handshake->upgrade_websocket || !conn->handshake->connection_upgrade) {
		nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Received uncomplete listener handshake reply (%p %d %d)", conn->handshake->websocket_accept,
				   conn->handshake->upgrade_websocket, conn->handshake->connection_upgrade);
		return nopoll_false;
	} /* end if */
	...
	if (!result) {
		nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Unable to accept connection Sec-Websocket-Accept %s is not expected %s, closing session", accept,
...
void nopoll_conn_complete_handshake_check(noPollConn* conn)
{
	...
	nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "calling to check handshake received on connection id %d role %d..", conn->id, conn->role);
...
int nopoll_conn_get_handshake_error()
{
	return errortype;
}
/**
...
int nopoll_conn_complete_handshake_client(noPollCtx* ctx, noPollConn* conn, char* buffer, int buffer_size)
{
	...
	if (!conn->handshake->received_101 && nopoll_ncmp(buffer, "HTTP/1.1 ", 9)) {
		...
		if (!nopoll_ncmp(buffer + iterator, "101", 3)) {
			if (nopoll_ncmp(buffer + iterator, "403", 3)) {
				errortype = 1;
			}
...
void nopoll_conn_complete_handshake(noPollConn* conn)
{
	...
	while (nopoll_true) {
		...
		if (buffer_size == 2 && nopoll_ncmp(buffer, "\r\n", 2)) {
			...
			if (errortype == 1) /*403 forbbiden*/
			{
				buffer_size = nopoll_conn_readline(conn, buffer, 1024);
				if (buffer_size > 0) {
					if (strstr(buffer, "{\"ssl_client_verify\":\"FAILED\"}") != NULL) {
						printf("get revoke flag \n");
						errortype = 2;
					}
				}
			}
...
noPollMsg* nopoll_conn_get_msg(noPollConn* conn)
{
	...
	if (conn->pending_ssl_accept) {
		...
		if (ssl_error == -1) {
			...
			switch (ssl_error) {
				case SSL_ERROR_WANT_READ:
					nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "still not prepared to continue because read wanted conn-id=%d (%p, session %d)", conn->id,
							   conn, conn->session);
					return NULL;
				case SSL_ERROR_WANT_WRITE:
					nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "still not prepared to continue because write wanted conn-id=%d (%p)", conn->id, conn);
					return NULL;
				default:
					break;
			} /* end switch */
		...
		nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Completed TLS operation from %s:%s (conn id %d)", conn->host, conn->port, conn->id);
	...
	if (conn->previous_msg) {
		...
		if (conn->previous_msg->payload_size > 0) {
			msg = nopoll_msg_new();
			if (msg == NULL) {
				nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "Failed to allocate memory for received message, closing session id: %d", conn->id);
				nopoll_conn_shutdown(conn);
				return NULL;
			} /* end if */
	...
	*/
	...
	if (bytes == 0) {
		...
		nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "Connection id=%d without data, errno=%d : %s, returning no message", conn->id, errno, strerror(errno));
		return NULL;
	}
	...
	if (bytes != 2) {
		...
		nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING,
				   "Expected to receive complete websocket frame header but found only %d bytes over conn-id=%d, saving to reuse later", bytes, conn->id);
		return NULL;
	} /* end if */
	...
	if (conn->role == NOPOLL_ROLE_LISTENER && !msg->is_masked) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "Received websocket frame with mask bit set to zero, closing session id: %d", conn->id);
	...
	if (msg->payload_size < 0) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "Received wrong payload size at first 7 bits, closing session id: %d", conn->id);
		nopoll_msg_unref(msg);
		nopoll_conn_shutdown(conn);
		return NULL;
	} /* end if */
	...
	if (msg->payload_size < 126) {
		...
		if (bytes != 2) {
			nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING,
					   "Failed to get next 2 bytes to read header from the wire, failed to received content, shutting down id=%d the connection", conn->id);
			nopoll_msg_unref(msg);
			nopoll_conn_shutdown(conn);
			return NULL;
		} /* end if */
		...
		if ((bytes = __nopoll_conn_receive(conn, buffer, 6)) != 6) {
			nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL,
					   "Expected to receive next 6 bytes for websocket frame header but found only %d bytes, closing session: %d", bytes, conn->id);
			nopoll_conn_shutdown(conn);
			return NULL;
		} /* end if */
	...
	if (msg->is_masked) {
		...
		if (bytes != 4) {
			...
			if (bytes >= 0 && nopoll_conn_is_ok(conn)) {
				nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING,
						   "Expected to receive incoming mask after header (4 bytes) but found %d bytes on conn-id=%d, saving %d for future operations ", bytes,
						   conn->id, conn->pending_buf_bytes);
				return NULL;
			} /* end if */

			nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL,
					   "Expected to receive incoming mask after header (4 bytes) but found %d bytes, shutting down id=%d the connection", bytes, conn->id);
			nopoll_conn_shutdown(conn);
			return NULL;
		} /* end if */
...
read_payload:
	...
	if (bytes < 0) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "Connection lost during message reception, dropping connection id=%d, bytes=%d, errno=%d : %s", conn->id,
	...
	if (bytes != msg->payload_size) {
		...
		nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "Received fewer bytes than expected (bytes: %d < payload size: %d)", bytes, (int) msg->payload_size);
	...
	if (bytes == 0 && msg == conn->previous_msg) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "bytes == %d, msg (%p) == conn->previous_msg (%p)", bytes, msg, conn->previous_msg);
		return NULL;
	} /* end if */

	/* now unmask content (if required) */
	if (msg->is_masked) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Unmasking (payload size %d, mask: %d, msg: %p, desp: %d)", msg->payload_size, nopoll_get_32bit(msg->mask),
				   msg, msg->unmask_desp);
...
int __nopoll_conn_send_common(noPollConn* conn, const char* content, long length, nopoll_bool has_fin, long sleep_in_header, noPollOpCode frame_type)
{
	...
	if (conn->role == NOPOLL_ROLE_CLIENT) {
		return nopoll_conn_send_frame(conn, /* fin */ has_fin, /* masked */ nopoll_true, frame_type, length, (noPollPtr) content, sleep_in_header);
	} /* end if */

	/* sending content as listener */
	return nopoll_conn_send_frame(conn, /* fin */ has_fin, /* masked */ nopoll_false, frame_type, length, (noPollPtr) content, sleep_in_header);
}
...
int nopoll_conn_read(noPollConn* conn, char* buffer, int bytes, nopoll_bool block, long int timeout)
{
	...
	if (conn->pending_msg) {
		...
		/* nopoll_log (conn->ctx, NOPOLL_LEVEL_DEBUG, "### ====> Read %d bytes from previous pending frame, requested %d. Pending diff %d", total_read, bytes,
		 * conn->pending_diff); */
	} /* end if */
	...
	while (nopoll_true) {
		...
		if (msg == NULL) {
			if (!nopoll_conn_is_ok(conn)) {
				nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "Received websocket conn-id=%d close during wait reply..", conn->id);
				if (total_read == 0 && !block)
					return -1;
				return total_read;
			} /* end if */
		...
		if (msg) {
			...
			/* return amount read */
			if (total_read == bytes || !block) {
				nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Finishing nopoll_conn_read because block=%d or total bytes requested=%d satisfied=%d ", block, bytes,
						   total_read);
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Finishing nopoll_conn_read timeout reached=%ld ms , returning total bytes requested=%d satisfied=%d ", timeout,
			   bytes, total_read);
...
int nopoll_conn_complete_pending_write(noPollConn* conn)
{
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "Found complete write operation didn't finish well, result=%d, errno=%d, conn-id=%d", bytes_written, errno,
			   conn->id);
	return bytes_written;
}
...
int nopoll_conn_flush_writes(noPollConn* conn, long timeout, int previous_result)
{
	...
	if (errno != NOPOLL_EWOULDBLOCK || nopoll_conn_pending_write_bytes(conn) == 0) {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "called flush but nothing is pending=%d or errno=%d isn't %d", nopoll_conn_pending_write_bytes(conn), errno,
				   NOPOLL_EWOULDBLOCK);
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "finishing flush operation, total written=%d, added to previous result=%d", total, previous_result);
 ...
 */
int nopoll_conn_send_frame(noPollConn* conn, nopoll_bool fin, nopoll_bool masked, noPollOpCode op_code, long length, noPollPtr content, long sleep_in_header)
...
/* define a random mask */
	...
	if (length < 126) {
		...
		header_size += 8;
		/* set length into the next bytes */
		nopoll_set_32bit(length, header + 6);
	} else {
		nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL,
				   "Unable to send the requested message, this requested is bigger than the value that can be supported by this platform (it should be < 65k)");
		return -1;
#endif
	}
	nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "nopoll_conn_send_frame 002\n");
	/* place mask */
	if (masked) {
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "nopoll_conn_send_frame 003\n");
	/* copy content to be sent */
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Copying into the buffer %d bytes of header (total memory allocated: %d)", header_size,
			   (int) length + header_size + 1);
	...
	nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "nopoll_conn_send_frame 004\n");
	/* send content */
	nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Mask used for this delivery: %d (about to send %d bytes)", nopoll_get_32bit(send_buffer + header_size - 2),
	...
	while (nopoll_true) {
		/* try to write bytes */
		nopoll_log(conn->ctx, NOPOLL_LEVEL_CRITICAL, "sleep_in_header %d 002\n", sleep_in_header);
		if (sleep_in_header == 0) {
			...
			if (bytes_written == header_size) {
				...
				nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Rest of content written %d (header size: %d, length: %d)", bytes_written, header_size, length);
				bytes_written = length + header_size;
				nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "final bytes_written %d", bytes_written);
			} else {
				nopoll_log(conn->ctx, NOPOLL_LEVEL_WARNING, "Requested to write %d bytes for the header but %d were written", header_size, bytes_written);
				return -1;
			} /* end if */
		}	 /* end if */
		...
		if ((bytes_written + desp) != (length + header_size)) {
			...
			nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Bytes written to the wire %d (masked? %d, mask: %u, header size: %d, length: %d)", bytes_written, masked,
					   mask_value, header_size, (int) length);
			break;
		} /* end if */
	...
	nopoll_log(conn->ctx, level, "Write operation finished with with last result=%d, bytes_written=%d, requested=%d, remaining=%d (conn-id=%d)",
	...
	if (conn->pending_write_bytes > 0) {
		...
		nopoll_log(conn->ctx, NOPOLL_LEVEL_DEBUG, "Stored %d bytes starting from %d out of %d bytes (header size: %d)", conn->pending_write_bytes,
				   length + header_size - conn->pending_write_bytes, length + header_size, header_size);
	} /* end if */
...
noPollConn* nopoll_conn_accept(noPollCtx* ctx, noPollConn* listener)
{
	...
	nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "Calling to accept web socket connection over master id=%d, socket=%d", listener->id, listener->session);
	...
	if (session <= 0) {
		nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Received invalid socket value from accept(2): %d, error code errno=: %d", session, errno);
		return NULL;
	} /* end if */
	...
	nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "Accepted new WebSocket conn-id=%d, socket=%d, over master id=%d, socket=%d", conn->id, conn->session, listener->id,
			   listener->session);
...
nopoll_bool nopoll_conn_accept_complete(noPollCtx* ctx, noPollConn* listener, noPollConn* conn, NOPOLL_SOCKET session, nopoll_bool tls_on)
{
	...
	if (ctx->on_accept) {
		...
		if (!ctx->on_accept(ctx, conn, ctx->on_accept_data)) {
			nopoll_log(ctx, NOPOLL_LEVEL_WARNING, "Application level denied accepting connection from %s:%s, closing", conn->host, conn->port);
			nopoll_conn_shutdown(conn);
			nopoll_ctx_unregister_conn(ctx, conn);
			return nopoll_false;
		} /* end if */
	...
	nopoll_log(ctx, NOPOLL_LEVEL_DEBUG, "Connection received and accepted from %s:%s (conn refs: %d, ctx refs: %d)", listener->host, listener->port,
			   listener->refs, ctx->refs);
	...
	if (listener->tls_on || tls_on) {
		...
		if (certificateFile == NULL || privateKey == NULL) {
			nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "Unable to accept secure web socket connection, certificate file %s and/or key file %s isn't defined",
					   certificateFile ? certificateFile : "<not defined>", privateKey ? privateKey : "<not defined>");
			nopoll_conn_shutdown(conn);
			nopoll_ctx_unregister_conn(ctx, conn);
			return nopoll_false;
		} /* end if */
