...
STRREMOTE_IPMAP remote_ipmap[] = {{SOCK_WATCHDOG_ID, "127.0.0.1", 7445, 7446},
								  {SOCK_BACKEND_ID, "127.0.0.1", 7433, 7434},
								  {SOCK_UPFW_ID, "127.0.0.1", 7650, 7651},
								  {SOCK_AC_ID, "127.0.0.1", 7635, 7636}};
					...
					if (0 == connect(client->fd, (struct sockaddr*) &(client->addr_in), sizeof(struct sockaddr))) {
						...
						if (llen == length) {
							DEBUGP(" %s ======test by mjx ===remote found in new "
								   "TCP connect pid:%d contype:%d state:%d "
